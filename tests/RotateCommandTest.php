 ...
 */
class RotateCommandTest extends AbstractBaseTest
{
    public function testFailure()
    {
        $this->expectException(\RuntimeException::class);
        $tester = $this->createCommandTester(new LogrotateRotateCommand());
        $tester->execute(['', '--no-interaction' => true]);

        $this->assertEquals(Command::FAILURE, $tester->getStatusCode(), 'Returns 0 in case of success');
    }
    ...
    public function testSuccess()
    {
        ...
        $this->assertEquals(Command::SUCCESS, $tester->getStatusCode(), 'Returns 1 in case of success');
    }
}
